...
public class Driver {
    private static void processCommand(String line, DoubleLinkedList<Contact> contactList) {
        ...
        switch (command) {
            case "addcontact":
                System.out.print("Adding Contact:     " + args[0] + ", " + args[1]);
                if (args.length < 3) {
                    System.out.println();
                } else {
                    System.out.println(", " + args[2]);
                }
                // TODO: Completete adding a contact - make sure email list is working ...
                Contact contact = new Contact(args[0], args[1]);
                for (int i = 2; i < args.length; i++) {
                    if (!args[i].isEmpty()) {
                        // TODO: add email here to the contect
            ...
            case "getemailaddress":
                if (args.length == 2) {
                    System.out.println("Emails list:        " + args[0] + ", " + args[1] + ":\t" +
                            // TODO: Implement getting email addresses for a contact
                            contactList.getEmailAddressesForContact(args[0], args[1])
                    );
                }
                break;
            case "isemptycontactlist":
                // TODO: Implement checking if the contact list is empty
                System.out.println("List is empty:        " + contactList.isEmpty());
                break;
            case "deleteemail":
                // TODO: Implement deleting an email from a contact
                System.out.println("Delete Email:        " + contactList.deleteEmailFromContact(args[0], args[1], args[2]));
                break;
            case "printcontacts":
